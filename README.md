...
Current status: package skeleton, OpenClaw channel wiring, XMPP conversation/target grammar, transport/login lifecycle, first-pass inbound handling, outbound text delivery, pairing approval notification, and direct-message OMEMO now exist. The plugin normalizes bare JIDs, understands explicit `dm:` and `room:` targets, connects/login with `@xmpp/client`, auto-joins configured rooms, routes direct messages through OpenClaw DM policy/pairing checks, routes groupchat messages with room allowlist and reply-policy gating, sends text replies back for inbound conversations, and can deliver outbound text to direct JIDs or rooms. Hardening work now also includes safer startup/shutdown behavior, normalized config allowlists, duplicate inbound message suppression, replay protection for older delayed room history after reconnects, and a focused Node 22 test harness covering target parsing, setup validation, access policy, delayed replay filtering, dedupe behavior, and transport lifecycle failures. Media is still basic: outbound media currently degrades to a plain text message with the media URL instead of native XMPP upload. Presence filtering is still basic, and live testing against `talk.ruzicka.us` is still TODO.
...
The current test harness is intentionally focused on high-risk behavior: target parsing, setup validation, sender/room policy decisions, delayed-history filtering, duplicate-message suppression, and transport lifecycle behavior such as failed starts, abort-driven shutdown, and stop-error surfacing.
...
4. Add deeper stanza-flow integration coverage and, once a container runtime is available, disposable-server coverage for reconnect, real stanza replay, and OMEMO optional/required flows.
